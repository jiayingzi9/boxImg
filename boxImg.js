;(function () {
    ...
    var _function={
        init:function(){
            _function.render.call(this);
        },
        render:function () {
            var len=$('.jf-boxImg>img').length;
            var arrPic = new Array(); //定义一个数组
            for (var i = 0; i < len; i++) {
                arrPic[i] = $('.jf-boxImg>img').eq(i).prop("src"); //将所有img路径存储到数组中
            };
            $('.jf-boxImg>img').each(function () {
            ...
            _function.boxStyle.call(this);
        },
        boxStyle:function () {
            var box=$('.mask-layer-container');
            return box.css({
                'width':this.element.boxImgWd,
                'height':this.element.boxImgHg,
            });
        },
    }
    var boxImg=function (element) {
    ...
    window.JF ? window.JF['boxImg']=boxImg : window['boxImg']=boxImg;
})();